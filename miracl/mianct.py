# Auto generated from miracl.yaml by pythongen.py version: 0.9.0
...
#              examples here: * [tests/input](https://github.com/cmungall/miracl-schema/tree/main/tests/input)
...
#              this relates to dosdp/robot templates When should one provide an entry for a cell type in a miracl
...
miracl = CurieNamespace("miracl", "https://w3.org/miracl/")
...
class NarrativeText(str):
    ...
    type_model_uri = miracl.NarrativeText
...
class CellType(YAMLRoot):
    ...
    class_model_uri: ClassVar[URIRef] = miracl.CellType
    ...
    develops_into: Optional[
        Union[Union[str, CellTypeId], List[Union[str, CellTypeId]]]
    ] = empty_list()
    ...
    def __post_init__(self, *_: List[str], **kwargs: Dict[str, Any]):
        ...
        if self.organ_or_tissue is not None and not isinstance(
            self.organ_or_tissue, str
        ):
        ...
        if self.major_life_stage is not None and not isinstance(
            self.major_life_stage, MajorLifeStageEnum
        ):
            self.major_life_stage = MajorLifeStageEnum(self.major_life_stage)

        if self.major_class is not None and not isinstance(
            self.major_class, MajorClassEnum
        ):
            self.major_class = MajorClassEnum(self.major_class)

        if self.diagnostic_description is not None and not isinstance(
            self.diagnostic_description, str
        ):
            self.diagnostic_description = str(self.diagnostic_description)

        if self.morphology is not None and not isinstance(
            self.morphology, MorphologyEnum
        ):
            self.morphology = MorphologyEnum(self.morphology)

        if self.develops_from is not None and not isinstance(
            self.develops_from, CellTypeId
        ):
            self.develops_from = CellTypeId(self.develops_from)

        if not isinstance(self.develops_into, list):
            self.develops_into = (
                [self.develops_into] if self.develops_into is not None else []
            )
        self.develops_into = [
            v if isinstance(v, CellTypeId) else CellTypeId(v)
            for v in self.develops_into
        ]
        ...
        self.previous_observations = [
            v if isinstance(v, str) else str(v) for v in self.previous_observations
        ]
...
class OrganismTaxon(YAMLRoot):
    ...
    class_model_uri: ClassVar[URIRef] = miracl.OrganismTaxon
...
class MajorClassEnum(EnumDefinitionImpl):
    ...
    neuron = PermissibleValue(text="neuron", meaning=CL["0000540"])
    leukocyte = PermissibleValue(text="leukocyte", meaning=CL["0000738"])
    ...
    def _addvals(cls):
        setattr(
            cls, "gial cell", PermissibleValue(text="gial cell", meaning=CL["0000125"])
        )
...
class MajorLifeStageEnum(EnumDefinitionImpl):
    ...
    def _addvals(cls):
        setattr(
            cls,
            "post-embryonic",
            PermissibleValue(text="post-embryonic", meaning=UBERON["0000092"]),
        )
        setattr(
            cls,
            "embryonic or fetal",
            PermissibleValue(text="embryonic or fetal", meaning=UBERON["0000068"]),
        )
        setattr(
            cls,
            "larval stage",
            PermissibleValue(text="larval stage", meaning=UBERON["0000069"]),
        )
